...
export async function POST(req: Request) {
  try {
    const { full_name, email, phone_number, company_name, role, password } = await req.json()
    const result = await signup({ full_name, email, phone_number, company_name, role, password })
